...
public:
    ...
    void removeEdge(int src, int dest) {
        ...
        adjList[dest].erase(remove_if(adjList[dest].begin(), adjList[dest].end(),
                            ...
                            adjList[dest].end());
        // Set distance to INF to simulate link failure
        dist[src][dest] = INF;
        dist[dest][src] = INF;
        nextHop[src][dest] = -1;
        nextHop[dest][src] = -1;
...
int main() {
    ...
    dvr.runDVRAlgorithmWithPoisonedReverse();
    ...
    // Re-run the DVR algorithm after link failure with Poisoned Reverse applied
    bool countToInfinity = dvr.runDVRAlgorithmWithPoisonedReverse();
    ...
    cout << "Routing Tables after link failure with Poisoned Reverse:\n";
    dvr.printRoutingTables();
